/**
 * @file float.c
 * @brief Parallel implementation to model the propagation of heat inside a cylindrical radiator.
...
__global__ void init_matrices_kernel(float* d_matrix, int n, int m){
	...
	if(i<n && j<m){
		int index = i*m+j;
		if(j==0){
			d_matrix[index] = 1.0f * (float)(i+1) / (float)(n);
		} else if(j==1){
			d_matrix[index] = 0.7f* (float)(i+1) / (float)(n);
		} else{
			d_matrix[index] = 1.0f / 15360.0f;
		}
	}
}

__global__ void propagate_heat_kernel(float* current_matrix, float* next_matrix, int n, int m){
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	int j = blockIdx.y * blockDim.y + threadIdx.y;
	...
	if(j<2){
		...
		next_matrix[i*m+j] = 1.65f * current_matrix[i*m+((j-2)+m)%m] +
				     		1.35f * current_matrix[i*m+((j-1)+m)%m] +
					    			current_matrix[i*m+j] +
				     		0.65f * current_matrix[i*m+(j+1)%m] +
					 		0.35f * current_matrix[i*m+(j+2)%m];
		next_matrix[i*m+j] /= 5.0f;	
	}

}

__global__ void swap_matrices_kernel(float *d_current, float *d_next, int n, int m){
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	int j = blockIdx.y * blockDim.y + threadIdx.y;

	if(i<n && j<m){
		int index = i*m+j;
		float temp = d_current[index];
...
__global__ void calculate_average_kernel(float* current_matrix, float* row_avg, int n, int m){
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	if(i<n){
		float row_sum = 0.0f;
		for(int j=0; j<m; j++){
...
int main(int argc, char **argv){
	...
	// GPU allocation
	float *d_current, *d_next, *d_row_avg;
	cudaEventRecord(start);
	cudaMalloc(&d_current, n*m*sizeof(float));
	cudaMalloc(&d_next, n*m*sizeof(float));
	cudaMalloc(&d_row_avg, n*sizeof(float));
	...
	float *row_avg_gpu = (float *)malloc(n*sizeof(float));
	cudaEventRecord(start);
	cudaMemcpy(row_avg_gpu, d_row_avg, n*sizeof(float), cudaMemcpyDeviceToHost);
 	cudaEventRecord(stop);
	cudaEventSynchronize(stop);
